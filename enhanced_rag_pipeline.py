#!/usr/bin/env python3
"""
Enhanced RAG Pipeline for SEO Blog Generation
...
class EnhancedRAGPipeline:
    ...
    async def generate_title(self, keyword: str) -> Dict[str, Any]:
        """SEO 최적화 제목 생성"""
...
다음 요구사항에 맞는 SEO 최적화된 블로그 제목을 생성해주세요:
...
"""
        ...
        if self.rag and self.rag.vs:
            rag_context = self.rag.query(keyword, k=2)
        ...
        # 토큰 추정 (실제 API에서는 usage 정보 활용)
        prompt_tokens = len(prompt.split()) * 1.3  # 추정
        ...
        try:
            ...
            if json_match:
                ...
                return {
                    "title": f"{keyword} 완벽 가이드 2025",
                    "seo_score": 7,
                    "character_count": len(f"{keyword} 완벽 가이드 2025"),
                }
        except:
            return {
                "title": f"{keyword} 완벽 가이드 2025",
    ...
    async def generate_keywords(self, keyword: str, title: str) -> Dict[str, List[str]]:
        """LSI 키워드와 롱테일 키워드 생성"""
        start_time = time.time()
...
1. LSI 키워드 10개 (Latent Semantic Indexing - 의미적으로 연관된 키워드)
2. 롱테일 키워드 8개 (구체적이고 길이가 긴 검색 키워드)

형식:
{{
  "lsi_keywords": ["키워드1", "키워드2", ...],
  "longtail_keywords": ["긴 키워드 구문1", "긴 키워드 구문2", ...]
}}
"""

        if self.rag and self.rag.vs:
            rag_context = self.rag.query(f"{keyword} 관련 키워드", k=2)
            prompt = f"{rag_context}\n\n{prompt}"
    ...
    async def generate_outline(
        self, keyword: str, title: str, keywords: Dict[str, List[str]]
    ) -> Dict[str, Any]:
        """블로그 아웃라인 생성"""
        start_time = time.time()

        all_keywords = ", ".join(
            keywords["lsi_keywords"][:5] + keywords["longtail_keywords"][:3]
        )
...
{{
  "sections": [
    {{
      ...
      "target_keywords": ["사용할 키워드1", "사용할 키워드2"]
    }}
  ],
  "total_estimated_length": 3000
}}
"""

        if self.rag and self.rag.vs:
            rag_context = self.rag.query(keyword, k=3)
            prompt = f"{rag_context}\n\n{prompt}"
        ...
        return {
            "sections": [
                {
                    "h2_title": f"{keyword}의 이해",
                    "content_points": ["기본 개념", "중요성", "적용 분야"],
                    "estimated_length": 600,
                    "target_keywords": keywords["lsi_keywords"][:2],
                },
                {
                    "h2_title": f"{keyword} 시작하기",
                    "content_points": ["준비 단계", "기본 방법", "주의사항"],
                    "estimated_length": 700,
                    "target_keywords": keywords["longtail_keywords"][:1],
                },
            ],
            "total_estimated_length": 2500,
        }

    async def generate_section_content(
        self, section: Dict[str, Any], keyword: str, context: str = ""
    ) -> str:
        """섹션별 콘텐츠 생성"""
        start_time = time.time()
        ...
        prompt = f"""
주요 키워드: {keyword}
섹션 제목: {section['h2_title']}
목표 키워드: {', '.join(section.get('target_keywords', []))}
내용 포인트: {', '.join(section['content_points'])}
목표 길이: {section['estimated_length']}자

다음 요구사항에 맞는 섹션 콘텐츠를 작성해주세요:
...
"""
        ...
        if self.rag and self.rag.vs:
            rag_context = self.rag.query(f"{keyword} {section['h2_title']}", k=2)
            prompt = f"{rag_context}\n\n{prompt}"
        ...
        self.track_llm_call(
            f"section_content_{section['h2_title'][:10]}",
            int(prompt_tokens),
            int(completion_tokens),
            duration,
            response.content,
            f"섹션 콘텐츠 생성: {section['h2_title']}",
        )
...
"""
    ...
    def create_html(self, title: str, sections_content: List[Dict[str, Any]]) -> str:
        """워드프레스용 HTML 생성 (H1 태그 제외)"""
        html_content = ""

        for section in sections_content:
            # H2 태그로 섹션 시작
            html_content += (
                f'<h2 class="blog-section-title">{section["h2_title"]}</h2>\n\n'
            )
            ...
            content = re.sub(
                r"^### (.+)$",
                r'<h3 class="blog-subsection">\1</h3>',
                content,
                flags=re.MULTILINE,
            )

            # 문단 처리 (줄바꿈 2개 이상을 기준으로 분할)
            paragraphs = content.split("\n\n")
            for para in paragraphs:
                para = para.strip()
                if para and not para.startswith("<h"):
                    html_content += f'<p class="blog-content">{para}</p>\n\n'
    ...
    # --- NEW: Single-call title+keywords generator ---
...
"""
    ...
    # --- NEW: Structure JSON with 5-7 H2, variable H3/H4 ---
...
"""
    ...
    # --- UPDATED: section content with teaser ---
    async def generate_section_with_context(
        ...
        # 티저 문장 스타일: '다음 섹션/챕터/자연스럽게 이어집니다' 등의 표현 금지. 암시적으로 다음 주제를 향하는 한 문장.
        teaser = (
...
"""
    ...
    async def run_complete_pipeline(self, keyword: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # HTML
            # 마크다운 → HTML은 표준 변환기를 사용해 제목/문단/헤딩을 정확히 처리
            from src.generators.html.simple_html_converter import (
                SimpleHTMLConverter,
            )
